...
export class PaymentsController implements OnModuleInit {
  ...
  onModuleInit() {
    this.svc = this.client.getService<PaySvc>("PaymentService");
  }

  @Post("intent") intent(@Body() body: any) {
    return firstValueFrom(this.svc.CreateIntent(body).pipe(timeout(5000)));
  }
}
